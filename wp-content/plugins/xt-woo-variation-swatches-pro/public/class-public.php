...
class XT_Woo_Variation_Swatches_Public {
	...
	public function __construct( &$core ) {
        ...
        add_filter( 'woocommerce_dropdown_variation_attribute_options_html', array( $this, 'variation_attribute_options_html'), 100, 2 );
        add_filter( 'xt_woovs_swatch_html', array( $this, 'swatch_html'), 5, 6 );
        add_filter( 'xt_woovs_swatch_meta_html', array( $this, 'swatch_meta_html'), 5, 4 );
        ...
        if(!is_admin()) {
            add_filter('wc_get_template', array($this, 'woocommerce_override_variable_template'), 10, 5);
        }

        add_filter('woocommerce_gallery_image_size', array( $this, 'woocommerce_gallery_image_size'), 10, 1);

        if($this->core->access_manager()->can_use_premium_code__premium_only()) {
            add_action('woocommerce_before_template_part', array( $this, 'woocommerce_before_template_part__premium_only'), 10, 1);
            add_action('woocommerce_after_template_part', array( $this, 'woocommerce_after_template_part__premium_only'), 10, 1);
            add_filter('woocommerce_loop_add_to_cart_args', array( $this, 'woocommerce_loop_add_to_cart_args__premium_only'), 10, 1);
            add_filter('woocommerce_product_variation_get_image_id', array( $this, 'woocommerce_product_variation_get_image_id__premium_only'), 10, 2);
            add_filter('woocommerce_variation_is_active', array($this, 'hide_out_of_stock__premium_only'), 10, 2);
        }
    }
    ...
    public function woocommerce_override_variable_template($template, $template_name, $args, $template_path, $default_path) {
	    ...
	    if($template_name === 'single-product/add-to-cart/variable.php') {
            ...
            $template = $this->core->get_template('variable', $args, null, true);

	    }else if($template_name === 'composited-product/variable-product.php') {

            $template = $this->core->get_template('composite-variable',$args, null, true);

        }else if($template_name === 'single-product/bundled-product-variable.php') {

            $template = $this->core->get_template('bundle-variable', $args, null, true);
        }
    ...
    public function woocommerce_before_template_part__premium_only($template_name)
    {
        ...
        if ($product) {
            ...
            if ($template_name === 'loop/add-to-cart.php') {
                ...
                if ($product->is_type('variable') && $this->enabled('archives')) {

                    if ($this->on_demand_enabled()) {
    ...
    public function woocommerce_after_template_part__premium_only($template_name) {
            ...
            if ($template_name === 'loop/add-to-cart.php') {
                ...
                if ($product->is_type('variable') && $this->enabled('archives')) {
                    ...
                    if ($this->on_demand_enabled()) {
                        ...
                        list($display_type, $visibility_type, $drawer_position) = $this->on_demand_settings();

                        $classes = ['xt_woovs-on-demand'];
                        $classes[] = 'xt_woovs-display-'.$display_type;

                        if(!empty($visibility_type)) {
                            $classes[] = 'xt_woovs-visibility-'.$visibility_type;
                        }
                        if(!empty($drawer_position)) {
                            $classes[] = 'xt_woovs-drawer-'.$drawer_position;
                        }

                        $classes = implode(" ", $classes);
                        ?>

                        <?php if (in_array($display_type, array('modal', 'drawer'))) : ?>
                            <span class="xt_woovs-display-backlayer"></span>
                        <?php endif; ?>

                        <div class="<?php echo esc_attr($classes);?>" data-display="<?php echo esc_attr($display_type);?>" data-visibility="<?php echo esc_attr($visibility_type);?>">
                            <span><?php echo esc_html($product->add_to_cart_text());?><span class="xt_woofcicon-close"></span></span>
                            <div class="xt_woovs-on-demand-wrap">

                        <?php if(in_array($display_type, array('modal', 'drawer'))): ?>
                                <div class="xt_woovs-on-demand-preview"></div>
                        <?php endif; ?>
    ...
    public function woocommerce_loop_add_to_cart_args__premium_only($args) {
        ...
        if ( $product ) {
            if ($product->is_type('variable') && $this->enabled('archives')) {
                ...
                $classes = array(
                    'button',
                    $prefix.'_select_options_button'
                );

                if(!empty($args['class'])) {
                    $classes = array_merge(explode(" ", $args['class']), $classes);
                }

                $display_type = xt_woovs_type_option('swatches_display_type', 'inline');
                $visibility_type = $display_type === 'inline' ? xt_woovs_type_option('swatches_visibility_type', 'always') : null;

                $classes[] = $prefix.'-display-' . $display_type;
                if(!empty($visibility_type)) {
                    $classes[] = $prefix.'-visibility-' . $visibility_type;
                }
    ...
    public function woocommerce_product_variation_get_image_id__premium_only($image_id, $product) {

	    $cache_key = $this->core->plugin_short_prefix('get_image_id_'.$product->get_id());

        $new_image_id = wp_cache_get( $cache_key );

        if(false === $new_image_id) {

            $attribute_image_preview = xt_woovs_type_option_bool('attribute_image_preview');

            if ($attribute_image_preview) {

                $attributes = $product->get_variation_attributes();

                $global_attributes = xt_woovs_type_option('attribute_image_preview_attributes');

                $global_attributes = XT_Framework_Customizer_Helpers::repeater_fields_string_to_array($global_attributes);
                ...
                foreach ($attributes as $attribute_name => $option) {
                    ...
                    if (!empty($option) && xt_woovs_search_attributes($global_attributes, 'attribute', $attribute_name) !== null) {
                ...
                if (!empty($found_attribute)) {
                    ...
                    $attribute = $found_attribute['attribute'];

                    if (!taxonomy_exists($attribute)) {
                        return $image_id;
                    }

                    $option = $found_attribute['option'];
                    $key = md5($attribute);
                    ...
                    if (!empty($term)) {
                        ...
                        $new_image_id = get_term_meta($term->term_id, 'image', true);

                        $product_swatch_options = $product->get_meta('_xt_woovs_swatch_type_options', true);

                        if(!empty($product_swatch_options)) {
                            $product_attr_options = $product_swatch_options[$key];

                            if ($product_attr_options['type'] === 'product_image') {

                                $product_attr_term_options = !empty($product_attr_options[$meta_key]) ? $product_attr_options[$meta_key] : null;

                                $new_image_id = !empty ($product_attr_term_options['image']) ? $product_attr_term_options['image'] : $new_image_id;
                            }

                            wp_cache_set( $cache_key, $new_image_id );
                        }
                    }
                }
            }
     ...
     * @since    1.0.0
     */
    public function hide_out_of_stock__premium_only($active, $variation) {
        ...
        $hide_out_of_stock = wc_string_to_bool( $this->core->customizer()->get_option_bool( 'hide_out_of_stock_variation', false ) );
    ...
    public function get_wrap_classes() {
        ...
        $wrap_classes[] = 'xt_woovs-disabled-' . xt_woovs_type_option('swatch_disabled_clickable', 'unclickable');
            ...
            if($product && taxonomy_exists( $attribute ) ) {
                ...
                foreach ( $terms as $term ) {
					...
					if ( in_array( $term->slug, $options ) ) {
                        ...
                        $meta_values = $this->get_global_attribute_term_options($args, $attr, $term, $product_swatch_options, $key, $meta_key);

                        $swatches .= apply_filters( 'xt_woovs_swatch_html', '', $term, $meta_values, $selected, $attributes, $product );
					}
				}
                ...
                foreach ($options as $option) {
                    ...
                    // Check if product has quick attributes with custom swatches, if yes use those instead.
                    $meta_values = $this->get_quick_attribute_term_options($args, $attr, $option, $product_swatch_options, $key, $meta_key);

                    $swatches .= apply_filters('xt_woovs_swatch_meta_html', '', $option, $meta_values, $selected);
    ...
    public function get_global_attribute_term_options(&$args, &$attr, &$term, &$product_swatch_options, $key, $meta_key) {

        $meta_values = $this->get_quick_attribute_term_options($args, $attr, $term->slug, $product_swatch_options, $key, $meta_key, false, $term);
        ...
        if(!$overridden) {
            $this->swatch_type_override($meta_values, $args, $attr, $term->slug, true);
        }

        return $meta_values;
    }

	public function get_quick_attribute_term_options(&$args, &$attr, &$option, &$product_swatch_options, $key, $meta_key, $enable_auto_convert = true, $term = null) {
        ...
        if($this->core->access_manager()->can_use_premium_code__premium_only()) {
            ...
            if (!empty($product_swatch_options[$key]) && $product_swatch_options[$key]['type'] !== 'default') {

                $product_attr_options = $product_swatch_options[$key];
                $product_attr_term_options = !empty($product_attr_options[$meta_key]) ? $product_attr_options[$meta_key] : null;

                $type = $product_attr_options['type'];

                if (!empty($product_attr_term_options)) {
                    $type = !empty($product_attr_term_options['type']) ? $product_attr_term_options['type'] : $type;
                }

                if ($type == 'product_color') {

                    $meta_values['type'] = 'color';
                    $meta_values['value'] = !empty ($product_attr_term_options['color']) ? $product_attr_term_options['color'] : '';

                } else if ($type == 'product_image') {

                    $meta_values['type'] = 'image';
                    $meta_values['value'] = !empty  ($product_attr_term_options['image']) ? $product_attr_term_options['image'] : '';
        ...
        if($enable_auto_convert && empty($meta_values['value'])) {
            $this->swatch_type_override($meta_values, $args, $attr, $option);
            $type = $meta_values['type'];
        }
        ...
        if ($type === 'color') {
            ...
            $meta_values['style'] = $this->get_option_value($product_attr_options, $product_attr_term_options, 'color_swatch_style', 'xt_woovs-round', $term);
            $meta_values['tooltip'] = $this->get_option_value($product_attr_options, $product_attr_term_options, 'color_swatch_tooltip', 'disabled', $term);

        } else if ($type === 'image') {

            $meta_values['image'] = $meta_values['value'];
            $meta_values['style'] = $this->get_option_value($product_attr_options, $product_attr_term_options, 'image_swatch_style', 'xt_woovs-round_corner', $term);
            $meta_values['tooltip'] = $this->get_option_value($product_attr_options, $product_attr_term_options, 'image_swatch_tooltip', 'disabled', $term);

        } else if ($type === 'label') {

            $meta_values['image'] = $this->get_variation_color_image($variations, $attribute_slug, $option);
            $meta_values['style'] = $this->get_option_value($product_attr_options, $product_attr_term_options, 'label_swatch_style', 'xt_woovs-square', $term);
            $meta_values['tooltip'] = $this->get_option_value($product_attr_options, $product_attr_term_options, 'label_swatch_tooltip', 'disabled', $term);
        ...
        if (in_array($type, array_keys($this->core->types))) {
            ...
            if($meta_values['tooltip'] === 'image') {
                $meta_values['tooltip_image'] = $this->get_option_value($product_attr_options, $product_attr_term_options, 'swatch_tooltip_image', null, $term);
            }

            if($meta_values['tooltip'] === 'text') {
                $meta_values['tooltip_text'] = $this->get_option_value($product_attr_options, $product_attr_term_options, 'swatch_tooltip_text', null, $term);
            }
        }
    ...
    public function get_option_value(&$product_attr_options, &$product_attr_term_options, $key, $default, $term) {
    ...
    public function swatch_type_override(&$meta_values, &$args, &$attr, $option, $is_term = false) {
        ...
        if(!empty(xt_woovs_type_option('color_to_image', true))){
            ...
            }else if(in_array($type, array('color', 'image'))){
	 ...
	 * @return string
	 */
	public function swatch_meta_html( $html, $option, $meta_values = array(), $selected = null ) {
		...
		switch ( $type ) {
			...
			case 'color':
				...
				$html = $this->get_color_swatch_html($option, $option, $value, $tooltip_data, $selected, $style);

				break;

			case 'image':

				$html = $this->get_image_swatch_html($option, $option, $value, $tooltip_data, $selected, $style);

				break;

			case 'label':

				$html = $this->get_label_swatch_html($option, $option, $tooltip_data, $selected, $style);
	 ...
	 * @return string
	 */
	public function swatch_html( $html, $term, $meta_values = array(), $selected = null, $attributes, $product) {

        $option  = esc_html( apply_filters( 'woocommerce_variation_option_name', $term->name, $term, $attributes, $product ) );
        $slug = $term->slug;
		...
		switch ( $type ) {
			...
			case 'color':
				...
				$html = $this->get_color_swatch_html($slug, $option, $value, $tooltip_data, $selected, $style);

				break;

			case 'image':

                $html = $this->get_image_swatch_html($slug, $option, $value, $tooltip_data, $selected, $style);

				break;

			case 'label':

				$html = $this->get_label_swatch_html($slug, $option, $tooltip_data, $selected, $style);
	...
	public function get_color_swatch_html($slug, $name, $color, $tooltip_data, $selected, $style) {
        ...
        if($this->core->access_manager()->can_use_premium_code__premium_only()) {
            ...
            $show_caption = xt_woovs_type_option_bool('color_show_caption', false);
            $tooltip_class = in_array($tooltip, array('image', 'text')) ? 'xt_woovs-has-tooltip' : '';

            if ($tooltip === 'image') {

                $size = xt_woovs_type_option('swatch_tooltip_image_size');
                $tooltip_value = $this->image_id_to_src($tooltip_image, array($size, $size));

            } else if ($tooltip === 'text') {

                $tooltip_value = $tooltip_text ? $tooltip_text : esc_attr($name);
            }
        }
	...
	public function get_image_swatch_html($slug, $name, $image, $tooltip_data, $selected, $style) {
        ...
        if($this->core->access_manager()->can_use_premium_code__premium_only()) {
            ...
            $show_caption = xt_woovs_type_option_bool('image_show_caption', false);
            $tooltip_class = in_array($tooltip, array('image', 'text')) ? 'xt_woovs-has-tooltip' : '';
	...
	public function get_label_swatch_html($slug, $name, $tooltip_data, $selected, $style) {
        ...
        if($this->core->access_manager()->can_use_premium_code__premium_only()) {
            ...
            if ($tooltip === 'image') {
                ...
                $tooltip_value = $tooltip_text ? $tooltip_text : esc_attr($slug);
            }
        }
