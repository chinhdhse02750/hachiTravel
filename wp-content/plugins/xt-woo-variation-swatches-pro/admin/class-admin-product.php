...
class XT_Woo_Variation_Swatches_Admin_Product {
    ...
    public function __construct( &$core ) {
        ...
        add_filter('woocommerce_product_data_tabs', array( $this, 'product_swatches_data_tab'), 99 , 1 );
        ...
        add_action('wp_ajax_xt_woovs_admin_tab_content', array( $this, 'load_tab_content_ajax') );

        add_action('save_post', array( $this, 'save_post_ajax') );
        add_action('admin_head-post.php', array( $this, 'enqueue_product_ajax_save_script') );
        add_action('admin_head-post-new.php', array( $this, 'enqueue_product_ajax_save_script') );
	...
	public function product_data_panel_wrap() {
        ...
        <div id="xt_woovs_tab_content" class="panel xt_woovs_tab_content woocommerce_options_panel hidden">
            <div class="xt_woovs_fields">
                <?php $this->load_tab_content($post->ID); ?>
            </div>
        </div>
	    <?php
	}
    ...
    function load_tab_content($post_id, $return = false)
    {
        ...
        if($this->core->access_manager()->can_use_premium_code__premium_only()) {
            ...
            $this->render_product_tab_content($post_id);

        }else{

            $this->core->admin_messages()->premium_required_big(esc_html__('product level swatch customizations that overrides global settings.', 'xt-woo-variation-swatches'));
        }
        ...
        if ( $attributes && count( $attributes ) > 0 ) :
            ...
            foreach ( $attribute_names as $name ) :
                ...
                <div class="field xt_woovs_meta_wrp">
                            ...
                            <tbody>
                                <tr>
                                    <td class="attribute_swatch_label">
                                        <strong>
                                            <span class="xt_woovs_edit_field row-title"><?php echo esc_html($current_label); ?></span>
                                            <span class="xt_woovs_badge"><?php echo ($current_is_taxonomy ? esc_html__( 'Global Attribute', 'xt-woo-variation-swatches') : esc_html__( 'Quick Attribute', 'xt-woo-variation-swatches')); ?></span>
                                        </strong>
                                    </td>
                                    <td class="attribute_swatch_type">
                                        <strong><?php echo esc_html__( 'Swatch Type', 'xt-woo-variation-swatches')?></strong>
                                        <select class="_xt_woovs_swatch_type_options_type" id="_xt_woovs_swatch_type_options_<?php echo esc_attr($key); ?>_type" name="<?php echo esc_attr($attr_meta_prefix);?>[type]">
                                            <option <?php selected( $current_type, 'default' ); ?> value="default"><?php echo esc_html__( 'Inherit Global', 'xt-woo-variation-swatches' ); ?></option>
                                            <option <?php selected( $current_type, 'product_color' ); ?> value="product_color"><?php echo esc_html__( 'Custom Colors', 'xt-woo-variation-swatches' ); ?></option>
                                            <option <?php selected( $current_type, 'product_image' ); ?> value="product_image"><?php echo esc_html__( 'Custom Images', 'xt-woo-variation-swatches' ); ?></option>
                                            <option <?php selected( $current_type, 'product_label' ); ?> value="product_label"><?php echo esc_html__( 'Custom Labels', 'xt-woo-variation-swatches' ); ?></option>
                                        </select>
                                    </td>
                                </tr>
                            </tbody>
                    ...
                    <div class="xt_woovs_fields xt_woovs_accordion <?php echo esc_attr($active_fields);?>">

                        <div class="field_option field_option_color section-color-swatch <?php echo esc_attr($active_color);?>">
                            <table class="xt_woovs_attribute_options widefat">
                                <tbody>
                                    <?php $this->render_options($attr_meta_prefix, $attribute_options, 'color', 'attribute'); ?>
                                </tbody>
                            </table>
                        </div>

                        <div class="field_option field_option_image <?php echo esc_attr($active_image);?>">

                            <table class="xt_woovs_attribute_options widefat">
                                <tbody>
                                    <?php $this->render_options($attr_meta_prefix, $attribute_options, 'image', 'attribute'); ?>
                                </tbody>
                            </table>

                        </div>

                        <div class="field_option field_option_label <?php echo esc_attr($active_label);?>">

                            <table class="xt_woovs_attribute_options widefat">
                                <tbody>
                                    <?php $this->render_options($attr_meta_prefix, $attribute_options, 'label', 'attribute'); ?>
                                </tbody>
                            </table>
                        ...
                        foreach ( $attribute_terms as $attribute_term ) :
                            ...
                            <div class="xt_woovs_field_meta xt_woovs_meta" data-id="<?php echo esc_attr($term_id);?>">
                                <div class="xs_woovs_accordion_handle">
                                    <table class="xt_woovs_input widefat">
                                        <tbody>

                                            <tr>
                                                <td width="30" class="xt_woovs_preview_td <?php echo esc_attr($hide_preview);?>">
                                                    <div class="field_option field_option_color section-color-swatch <?php echo esc_attr($active_color);?>">
                                                        <div class="xt_woovs_preview xt_woovs_color_preview" <?php echo esc_attr($color_preview); ?>></div>
                                                    </div>

                                                    <div class="field_option field_option_image xt_woovs_image_picker <?php echo esc_attr($active_image);?>">
                                                        <div class="xt_woovs_preview xt_woovs_image_preview" <?php echo esc_attr($image_preview); ?>></div>
                                                    </div>
                                                </td>
                                                ...
                                                <td class="attribute_swatch_type">
                                                    ...
                                                    <select class="_xt_woovs_swatch_term_type_options_type" id="_xt_woovs_swatch_term_type_options_<?php echo esc_attr($key); ?>_<?php echo esc_attr( $attribute_term['id'] ); ?>_type" name="<?php echo esc_attr($term_meta_prefix); ?>[type]">
                                                        <option <?php selected( $current_type, 'product_color' ); ?> value="product_color"><?php echo esc_html__( 'Color', 'xt-woo-variation-swatches' ); ?></option>
                                                        <option <?php selected( $current_type, 'product_image' ); ?> value="product_image"><?php echo esc_html__( 'Image', 'xt-woo-variation-swatches' ); ?></option>
                                                        <option <?php selected( $current_type, 'product_label' ); ?> value="product_label"><?php echo esc_html__( 'Label', 'xt-woo-variation-swatches' ); ?></option>
                                                    </select>
                                            ...
                                            <tr>
                                                <td colspan="2">
                                                    ...
                                                    <div class="field_option field_option_color section-color-swatch <?php echo esc_attr($active_color);?>">
                                                        <table class="xt_woovs_input widefat">
                                                            <tbody>
                                                                <tr>
                                                                    <td class="label" width="25%">
                                                                        <?php echo esc_html__( 'Color')?>
                                                                    </td>
                                                                    <td>
                                                                         <input type="text" class="xt_woovs-term-color xt_woovs-color-picker" name="_xt_woovs_swatch_type_options[<?php echo esc_attr($key); ?>][<?php echo esc_attr( $attribute_term['id'] ); ?>][color]" value="<?php echo esc_attr( $color ); ?>">
                                                                    </td>
                                                                </tr>
                                                                <?php $this->render_options($term_meta_prefix, $term_options, 'color', 'term'); ?>
                                                            </tbody>
                                                        </table>
                                                    </div>
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        <div class="xt_woovs_image_picker">
                                                                            <img src="<?php echo esc_url( $image_picker_preview ) ?>" width="60px" height="60px" />

                                                                            <input type="hidden" class="xt_woovs-term-image" name="_xt_woovs_swatch_type_options[<?php echo esc_attr($key); ?>][<?php echo esc_attr( $attribute_term['id'] ); ?>][image]" value="<?php echo esc_attr( $img_id ) ?>" />

                                                                            <a href="#" class="button xt_woovs-meta-uploader" data-uploader-title="<?php echo sprintf(esc_html__('Add image(s) to %s', 'xt-woo-variation-swatches'), esc_attr( $attribute_term['label'] )); ?>" data-uploader-button-text="<?php echo esc_html__('Add image(s)', 'xt-woo-variation-swatches'); ?>"> <?php echo esc_html__( 'Upload/Add image', 'xt-woo-variation-swatches' ); ?></a>
                                                                            <a href="#" class="button xt_woovs_remove_meta_img <?php echo esc_attr($remove_image_hidden);?>"><?php echo esc_html__( 'Remove image', 'xt-woo-variation-swatches' ); ?></a>
                                                                        </div>
                                                                    </td>
                                                                </tr>
                                                                <?php $this->render_options($term_meta_prefix, $term_options, 'image', 'term'); ?>
                                                            </tbody>
                                                        </table>
                                                    ...
                                                    <div class="field_option field_option_label <?php echo esc_attr($active_label);?>">
                                                        ...
                                                        <table class="xt_woovs_input widefat">
                                                            <tbody>
                                                                <?php $this->render_options($term_meta_prefix, $term_options, 'label', 'term'); ?>
                                                            </tbody>
                                                        </table>
    ...
    public function render_options($key_prefix, $options, $type, $scope) {
        ...
        foreach($this->fields as $field) {
            ...
            ?>
            <tr class="xt_woovs_field_row <?php echo esc_attr($visibility);?>" data-type="<?php echo esc_attr($type);?>" data-type_prefix="<?php echo esc_attr($type_prefix);?>" data-key_prefix="<?php echo esc_attr($key_prefix);?>" data-field="<?php echo esc_attr($meta_key);?>" data-conditions="<?php echo htmlspecialchars(json_encode($visibleConditions), ENT_QUOTES, 'UTF-8'); ?>">
                <td class="label" width="25%">
                ...
                if($field['type'] === 'select') {
                    ...
                    $this->render_dropdown_field($field, $key_prefix, $meta_key, $value);

                }else if($field['type'] === 'text') {

                    $this->render_text_field($field, $key_prefix, $meta_key, $value);

                }else if($field['type'] === 'image') {

                    $this->render_image_field($field, $key_prefix, $meta_key, $value);
                }
                ?>
                </td>
            </tr>
    ...
    public function render_dropdown_field($field, $key_prefix, $key, $value) {

        $options = $field['options'];
        ?>
        <select class="xt_woovs_field" name="<?php echo esc_attr($key_prefix); ?>[<?php echo esc_attr($key);?>]">

            <option <?php selected( $value, null ); ?> value="">Inherit</option>

            <?php foreach($options as $option_key => $option): ?>
                <option <?php selected( $value, $option_key ); ?> value="<?php echo esc_attr($option_key);?>"><?php echo esc_html($option);?></option>
            <?php endforeach; ?>
    ...
    public function render_text_field($field, $key_prefix, $key, $value) {

        ?>
        <input class="xt_woovs_field" name="<?php echo esc_attr($key_prefix); ?>[<?php echo esc_attr($key);?>]" value="<?php echo esc_attr($value);?>" />
        <?php
    }

    public function render_image_field($field, $key_prefix, $key, $value) {
        ...
        <div class="xt_woovs_image_picker">
            ...
            <input type="hidden" class="xt_woovs_field xt_woovs-term-image" name="<?php echo esc_attr($key_prefix); ?>[<?php echo esc_attr($key);?>]" value="<?php echo esc_attr($value);?>" />

            <a href="#" class="button xt_woovs-meta-uploader" data-uploader-title="<?php echo sprintf(esc_html__('Add image(s) to %s', 'xt-woo-variation-swatches'), esc_attr( $label )); ?>" data-uploader-button-text="<?php echo esc_html__('Add image(s)', 'xt-woo-variation-swatches'); ?>"> <?php echo esc_html__( 'Upload/Add image', 'xt-woo-variation-swatches' ); ?></a>
            <a href="#" class="button xt_woovs_remove_meta_img <?php echo esc_attr($remove_image_hidden);?>"><?php echo esc_html__( 'Remove image', 'xt-woo-variation-swatches' ); ?></a>
    ...
    public function isFieldVisible($type, &$field, &$options) {
        ...
        foreach($field['visible'] as $condition) {
            ...
            }else if($op == 'in' && is_array($target_value) && !in_array($current_value, $target_value)) {
    ...
    function enqueue_product_ajax_save_script()
    {
        ...
        if ($post->post_type == 'product')
        {
            ...
            wp_enqueue_script( $this->core->plugin_slug('ajax-product-save'), $this->core->plugin_url( 'admin' ) . 'assets/js/product-edit'.XTFW_SCRIPT_SUFFIX.'.js', array( 'jquery' ), $this->core->plugin_version(), false );
