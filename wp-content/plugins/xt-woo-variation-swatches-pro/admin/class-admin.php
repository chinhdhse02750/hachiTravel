...
class XT_Woo_Variation_Swatches_Admin
{
	...
	public function __construct(&$core)
	{
        ...
        add_action( 'xt_woovs_product_attribute_field', array( $this, 'attribute_fields'), 10, 5);
	...
	public function enqueue_scripts()
	{
		...
		wp_enqueue_media();
		wp_register_script($this->core->plugin_slug('admin'), $this->core->plugin_url('admin/assets/js', 'admin' . XTFW_SCRIPT_SUFFIX . '.js'), array('jquery', 'wp-color-picker', 'wp-util'), $this->core->plugin_version(), true);
    ...
    public function init_attribute_hooks() {
        ...
        add_action( 'quick_edit_custom_box', array( $this, 'quick_edit_attribute_type_field'), 10, 2 );
    }
    ...
    public function add_attribute_fields( $taxonomy ) {
        ...
        do_action( 'xt_woovs_product_attribute_field', $attr->attribute_type, $attr->attribute_type, ucfirst($attr->attribute_type), '', 'add' );
        do_action( 'xt_woovs_product_attribute_field', 'tooltip', $attr->attribute_type.'_swatch_tooltip', 'Tooltip', '', 'add' );
    }
    ...
    public function edit_attribute_fields( $term, $taxonomy ) {
        ...
        do_action( 'xt_woovs_product_attribute_field', $attr->attribute_type, $attr->attribute_type, ucfirst($attr->attribute_type), $value, 'edit' );
        do_action( 'xt_woovs_product_attribute_field', 'tooltip', $attr->attribute_type.'_swatch_tooltip', 'Tooltip', $tooltip, 'edit' );

        if($tooltip === 'image') {
            $tooltip_image = get_term_meta( $term->term_id, 'swatch_tooltip_image', true );
            do_action('xt_woovs_product_attribute_field', 'tooltip_image', 'swatch_tooltip_image', 'Tooltip Image', $tooltip_image, 'edit');
        }else if ($tooltip === 'text') {
            $tooltip_text = get_term_meta( $term->term_id, 'swatch_tooltip_text', true );
            do_action('xt_woovs_product_attribute_field', 'tooltip_text', 'swatch_tooltip_text', 'Tooltip Text', $tooltip_text, 'edit');
        }
    }
     ...
     * @param $form
     */
    public function attribute_fields( $id, $meta_key, $label, $value, $form ) {
    ...
    function quick_edit_attribute_type_field( $column_name, $screen) {
        ...
        <fieldset>
            <div id="gwp-first-appeared" class="inline-edit-col">
                <label>
                    <span class="title"><?php echo esc_html__('Swatch', 'xt-woo-variation-swatches');?></span>
                    <span class="input-text-wrap">
	                	<?php $this->attribute_swatch_type_field($attr->attribute_type, $attr->attribute_type); ?>
	                </span>
                </label>
                <label>
                    <span class="title"><?php echo esc_html__('Tooltip', 'xt-woo-variation-swatches');?></span>
                    <span class="input-text-wrap">
	                	<?php $this->attribute_swatch_type_field('tooltip', $attr->attribute_type.'_swatch_tooltip'); ?>
	                </span>
                </label>
                <label class="xt_woovs-inline-edit-hidden">
                    <span class="title"><?php echo esc_html__('Tooltip Image', 'xt-woo-variation-swatches');?></span>
                    <span class="input-text-wrap">
	                	<?php $this->attribute_swatch_type_field('tooltip_image', 'swatch_tooltip_image'); ?>
	                </span>
                </label>
                <label class="xt_woovs-inline-edit-hidden">
                    <span class="title"><?php echo esc_html__('Tooltip Text', 'xt-woo-variation-swatches');?></span>
                    <span class="input-text-wrap">
	                	<?php $this->attribute_swatch_type_field('tooltip_text', 'swatch_tooltip_text'); ?>
	                </span>
                </label>
            </div>
        </fieldset>
        ...
        switch ( $id ) {
            case 'image':
                ...
                <div class="xt_woovs_image_picker swatch_image">
                    ...
                    <input type="hidden" class="xt_woovs-term-image" name="<?php echo esc_attr($meta_key);?>" value="<?php echo esc_attr( $value ) ?>" />
                    <a href="#" class="button xt_woovs-meta-uploader" data-uploader-title="<?php echo esc_html__( 'Add image to Attribute ', 'xt-woo-variation-swatches' ); ?>" data-uploader-button-text="<?php echo esc_html__( 'Add image to Attribute ', 'xt-woo-variation-swatches' ); ?>  "> <?php echo esc_html__( 'Upload/Add image', 'xt-woo-variation-swatches' ); ?></a>
                    <a href="#" class="xt_woovs_remove_meta_img button <?php echo esc_attr($image_remove_hidden);?>"><?php echo esc_html__( 'Remove image', 'xt-woo-variation-swatches' ); ?></a>
                </div>
            ...
            case 'color':
                ?>
                <input type="text" class="xt_woovs-term-<?php echo esc_attr( $meta_key ) ?> xt_woovs-color-picker" name="<?php echo esc_attr( $meta_key ) ?>" value="<?php echo esc_attr( $value ) ?>" />
                <?php
                break;
            ...
            case 'tooltip':
                ...
                if(!$this->core->access_manager()->can_use_premium_code__premium_only()) {
                    ...
                    ?>
                    <select type="text" class="xt_woovs-select xt_woovs-tooltip-select xt_woovs-term-<?php echo esc_attr($meta_key) ?>" name="<?php echo esc_attr($meta_key) ?>">
                        <option <?php selected($value, null ); ?> value="">Inherit</option>
                        <option <?php selected($value, 'disabled'); ?> value="disabled"><?php echo esc_html__('Disabled', 'xt-woo-variation-swatches');?></option>
                        <option <?php selected($value, 'text'); ?> value="text"><?php echo esc_html__('Text', 'xt-woo-variation-swatches');?></option>
                        <option <?php selected($value, 'image'); ?> value="image"><?php echo esc_html__('Image', 'xt-woo-variation-swatches');?></option>
                    </select>
                    <?php
                }
                break;
            ...
            case 'tooltip_image':
                ...
                if(!$this->core->access_manager()->can_use_premium_code__premium_only()) {
                    ...
                    $image_remove_hidden = $image ? '' : 'hidden';
                    ?>
                    <div class="xt_woovs_image_picker tooltip_image">
                        <img src="<?php echo esc_url($image_preview) ?>" width="60px" height="60px"/>
                        <input type="hidden" class="xt_woovs-term-image" name="<?php echo esc_attr($meta_key); ?>" value="<?php echo esc_attr($value) ?>"/>
                        <a href="#" class="button xt_woovs-meta-uploader" data-uploader-title="<?php echo esc_html__('Add tooltip image', 'xt-woo-variation-swatches'); ?>" data-uploader-button-text="<?php echo esc_html__('Add tooltip image', 'xt-woo-variation-swatches'); ?>"><?php echo esc_html__('Upload/Add image', 'xt-woo-variation-swatches'); ?></a>
                        <a href="#" class="xt_woovs_remove_meta_img button <?php echo esc_attr($image_remove_hidden); ?>"><?php echo esc_html__('Remove image', 'xt-woo-variation-swatches'); ?></a>
                    </div>
                    <div class="xt_woovs-clearfix"></div>
                    <?php
            ...
            case 'tooltip_text':
                ...
                if(!$this->core->access_manager()->can_use_premium_code__premium_only()) {
                    ...
                    ?>
                    <input type="text" class="xt_woovs-term-<?php echo esc_attr($meta_key) ?>" name="<?php echo esc_attr($meta_key) ?>" value="<?php echo esc_attr($value) ?>"/>
                    <?php
                }
                break;
    ...
    public function render_upgrade_notice() {
        ...
        <div>
            <span>
                <strong><?php echo __('Premium Feature!', 'xt-woo-variation-swatches');?></strong>
                <a href="<?php echo esc_url($this->core->access_manager()->get_upgrade_url());?>"><?php echo __('Upgrade to Unlock!', 'xt-woo-variation-swatches'); ?></a>
            </span>
        </div>
        <br>
        <?php
    ...
    public function product_option_terms( $taxonomy, $index ) {
        ...
        <select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'xt-woo-variation-swatches' ); ?>" class="multiselect attribute_values wc-enhanced-select" name="attribute_values[<?php echo esc_attr($index); ?>][]">
            <?php
        ...
        <button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'xt-woo-variation-swatches' ); ?></button>
        <button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'xt-woo-variation-swatches' ); ?></button>
    ...
    public function render_attribute_swatch_type_field_value($term_id, $id, $meta_key) {
        ...
        switch ( $id ) {
            case 'color':
                ...
                echo sprintf(
                    '<div class="swatch-preview swatch-color" style="background-color:%1$s;"></div>',
                    esc_attr( $value )
                );
                break;
            ...
            case 'tooltip_image':
                ...
                echo sprintf(
                    '<input class="swatch-tooltip-image-input" type="hidden" data-url="%1$s" value="%2$s" />',
                    esc_url($image),
                    esc_textarea($value)
                );
                break;
