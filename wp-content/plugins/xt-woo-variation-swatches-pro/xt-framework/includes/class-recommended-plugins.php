...
if ( ! class_exists( 'XT_Framework_Recommended_Plugins' ) ) {
    ...
    class XT_Framework_Recommended_Plugins {
        ...
        public function __construct( $core ) {
            ...
            add_filter('install_plugins_table_api_args_xt_plugins', array( $this, 'native_plugins_args'), 10, 1);
            add_action('install_plugins_xt_plugins',  array( $this, 'browse_plugins') );
            add_action('network_install_plugins_xt_plugins',  array( $this, 'browse_plugins') );
        ...
        public function is_plugin_install_page() {
            ...
            return (!empty($screen) && ($screen->id === 'plugin-install' || $screen->id === 'plugin-install-network') && $tab !== 'xt_plugins');
        }

        public function enqueue_assets() {

            $is_xt_plugins_page = $this->core->framework_tabs()->is_tab($this->slug);

            if(!$is_xt_plugins_page && !$this->is_plugin_install_page()) {
                return;
            }
        ...
        public function add_native_plugins_tab($tabs) {
            ...
            $tabs['xt_plugins'] = esc_html__( 'XT Plugins', 'xt-framework' );
        ...
        public function get_badges() {
            ...
            $total = $this->core->transient()->result('get_total_plugins', function() {

                $table = $this->get_plugins_table();
                $items = !empty($table) ? $table->items : array();
        ...
        function search_terms() {
            return array('xplodedthemes', 'woocommerce', 'cart', 'quick view', 'quickview', 'points', 'rewards', 'swatches', 'attributes', 'variation', 'variations');
        }
        ...
        public function alter_plugin_results( $plugins ) {
            ...
            $plugins = array_filter($plugins, function($plugin) {
                ...
                return empty($plugin['duplicated']);
            });
